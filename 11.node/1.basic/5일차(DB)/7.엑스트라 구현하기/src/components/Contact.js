...
class Contact extends React.Component{
  ...
  handleEdit(name,phone){//데이터수정(~,addr)
    this.setState({      //1.배열의 키명
      contactData:update(this.state.contactData,{
        //2.[수정할값에 대한 인덱스번호]
        [this.state.selectedKey]:{
          name:{$set:name},  //형식) name(키명):{$set:저장할값}
          //키명:{$set:'테스트김(수정할값)'}  홍길동->테스트김
          phone:{$set:phone}//,addr:{$set:'변경된 주소'},,
        }
      })
    })
  }
  handleRemove(){//데이터삭제
    //추가로 선택하지 않았으면 삭제되지 않도록 설정
    if(this.state.selectedKey < 0){
      return;//탈출문=>문장을 안쓰면 else로 묶어줘야 된다.
    }
     this.setState({ //this.state.속성명=값(X) 캡슐화
       contactData:update(this.state.contactData,
        //$splice메서드를 이용해서 선택된키부터 몇개를 삭제?
        //삭제할 대상자도 배열로 묶어준다.[]
        {$splice:[[this.state.selectedKey,1]]}),
        /*
          index번호가 0(홍길동),1 =>0번째 데이터삭제
          index번호가 0(홍길동),2 =>0,1번째 데이터 삭제
          index번호가 0(홍길동),3 =>0,1,2번째 데이터 삭제
        */
        selectedKey:-1 //다시 삭제시킬 대상자를 선택하기위해서 초기화
     })
  }

  /*-render()함수전에 호출되는 초기화함수
     (DB에 저장된 데이터가져와서 출력 */
  componentWillMount(){ //DB->화면에 출력=>DB=화면결과
    //const 변수명=localStorage.불러올 키명
    const contactData=localStorage.contactData
    if(contactData){
      this.setState({
        contactData:JSON.parse(contactData)//문자열->배열객체로변환
      })
    }
    console.log('componentWillMount()호출됨!')
  }
  //화면에 데이터를 수정(추가,수정,삭제)할때 호출
  componentDidUpdate(prevProps,prevState){
    //이미 저장된 과거의데이터와 현재 데이터가 다르면
    //DB(객체로 저장)->문자로 변경해서 확인
    if(JSON.stringify(prevState.contactData)!=JSON.stringify(this.state.contactData)){
      //전의 로컬저장소에 다시 새로운 내용으로 변경하라->
      //형식)localStorage.동적키명=저장할값(문자열로 변환해서 저장)
